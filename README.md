...
- 支持连接独立后端 `PotatoCircleBackend`，读取真实线上社区的信息流。
- 支持新用户使用头像、用户名和系统通行密钥完成注册，不使用传统密码登录。
- 用户设备标识只用于服务端生成私有哈希，不作为公开用户 ID 展示。
- 圈子图片在 App 内保持锁定状态，不直接预览；用户点击传输时才申请短时票据并传输至土豆片设备。
- 支持选择本地图片发布圈子作品，服务端将原图写入私有对象存储。

当前限制：

- 头像实际对象上传仍待补齐，目前注册流程只提交头像 Key。
- 老用户登录 UI 尚未接入，后端已具备 Passkey 登录接口。
- 当前默认连接内网服务 `http://192.168.100.112:3000`；迁移线上时需要替换为 HTTPS 服务地址。
...
✅ 不包含本地环境文件  
✅ 不采集用户隐私数据  
✅ 除用户主动发布圈子作品外，不上传用户图片内容
✅ 圈子作品上传后使用私有对象存储，信息流不暴露图片直链
